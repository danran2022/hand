...
class HandDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'intr' in sample.keys():
            ...
            affinetrans_nochange, post_rot_trans_nochange = handutils.get_affine_transform(
                center=center,
                scale=scale,
                optical_center=[intr[0, 2], intr[1, 2]],
                out_res=[self.inp_res, self.inp_res],
        ...
        # clr_before_Transform = np.array(clr)
        # Transform and crop
        ...
        # clr_pro = torch.tensor(clr)
        clr_pro = np.array(clr)

        clr_no_change = handutils.transform_img(
            my_clr1, affinetrans_nochange, [self.inp_res, self.inp_res]
        )
        clr_no_change = clr_no_change.crop((0, 0, self.inp_res, self.inp_res))
        # my_clr2 = clr.copy()
        clr_no_change = np.array(clr_no_change)
        ...
        ''' implicit HWC -> CHW, 255 -> 1 '''
        clr_no_change_nor = func.to_tensor(clr_no_change).float()
        ''' 0-mean, 1 std,  [0,1] -> [-0.5, 0.5] '''
        clr_no_change_nor = func.normalize(clr_no_change_nor, [0.5, 0.5, 0.5], [1, 1, 1])
        ...
        metas = {
            ...
            'clr': clr,
            "clr_pro": clr_pro,
            "clr_no_change_nor": clr_no_change_nor,
            "clr_no_change": clr_no_change,
            "img_paths": img_paths,
            'hm': hm,
            'hm_veil': hm_veil,
            ...
            "joint": joint * 100.0,
            "joint_nor": joint_nor,
            "joint_pro": joint_pro * 100.0,
            "joint_no_change_nor": joint_no_change_nor,
            "center_pro": sample['center'],
            "scale_pro": sample['my_scale'],
            "intr_pro": sample['intr'],
            "rot_angle": rot_angle,
            "rot_mat_all": rot_mat_all,
